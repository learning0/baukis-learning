...
feature '職員による顧客電話番号管理' do
  ...
  scenario '職員が顧客の支度電話番号を追加する' do
    click_link '顧客管理'
    first('table.listing').click_link '編集'
    
    fill_in 'form_home_address_phones_0_number', with: '030-9999-9999'
    check 'form_home_address_phones_0_primary'
    click_button '更新'
    
    customer.reload
    expect(customer.home_address.phones.size).to eq(1)
    expect(customer.home_address.phones[0].number).to eq('030-9999-9999')
  end
end